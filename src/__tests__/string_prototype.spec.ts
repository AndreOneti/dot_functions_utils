...
describe("String prototype test", () => {
  ...
  it("should return all strings captalized", async () => {
    expect("my string".splitToArray(3)).toEqual(
      expect.arrayContaining(["my ", "str", "ing"])
    );
  });

  it("should return all strings captalized", async () => {
    expect("string".splitToArray(3)).toEqual(
      expect.arrayContaining(["str", "ing"])
    );
  });
  ...
  it("should return string upperCase", async () => {
    let name = "AndreOneti";
    let expected = "ANDREONETI";
    let result = name.map((letter) => letter.toUpperCase());
    expect(result).toBe(expected);

    name = "My name is AndreOneti";
    expected = "MY NAME IS ANDREONETI";
